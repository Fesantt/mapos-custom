...
class Os_model extends CI_Model
{
    ...
    {
        $procura = ["{CLIENTE_NOME}", "{NUMERO_OS}", "{STATUS_OS}", "{VALOR_OS}", "{DESCRI_PRODUTOS}", "{EMITENTE}", "{TELEFONE_EMITENTE}", "{OBS_OS}", "{DEFEITO_OS}", "{LAUDO_OS}", "{DATA_FINAL}", "{DATA_INICIAL}", "{DATA_GARANTIA}", "{TOKEN_OS}", "{URL}"];
    ...
    // **** SANTT ****
//VISUALIZAR ORDEM DE SERVIÇO POR TOKEN
    public function getByToken($tokenOs)
        {
             $osData = $this->db->get_where('os', ['tokenOs' => $tokenOs], 1)->row();
         if ($osData) {
            $this->db->select('os.*, clientes.*, clientes.celular as celular_cliente, clientes.telefone as telefone_cliente, clientes.contato as contato_cliente, garantias.refGarantia, usuarios.telefone as telefone_usuario, usuarios.email as email_usuario, usuarios.nome');
            $this->db->from('os');
            $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
            $this->db->join('usuarios', 'usuarios.idUsuarios = os.usuarios_id');
            $this->db->join('garantias', 'garantias.idGarantias = os.garantias_id', 'left');
            $this->db->where('os.tokenOs', $tokenOs);
            $this->db->limit(1);
            $osData = $this->db->get()->row();
            $osData->produtos = $this->getProdutosByToken($tokenOs);
            return $osData;
        } else {
            return null;
        }
    ...
    public function getProdutosByToken($tokenOs)
    {
        $this->db->select('produtos_os.*, produtos.*');
        $this->db->from('produtos_os');
        $this->db->join('produtos', 'produtos.idProdutos = produtos_os.produtos_id');
    ...
    public function getServicosByToken($tokenOs)
    {
    ...
    public function getAnexosbytoken($tokenOs)
    {
        $this->db->where('token', $tokenOs);
        return $this->db->get('anexos')->result();
    }

    public function addAnotacao($data)
    {
        $this->db->insert('anotacoes_os', $data);

        // Optionally, you can check if the insertion was successful
        return $this->db->affected_rows() > 0;
    }
    // **** SANTT ****
    // FIM DO VISUALIZAR ORDEM DE SERVIÇO POR TOKEN 
