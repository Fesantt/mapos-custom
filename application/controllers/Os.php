...
class Os extends MY_Controller
{
    ...
    public function adicionar()
    {
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            $tokenOs = bin2hex(random_bytes(4)); 

            $data = [
                'dataInicial' => $dataInicial,
                'tokenOs' => set_value('tokenOs', $tokenOs),
    ...
    public function visualizar()
    {
        ...
        $this->load->model('mapos_model');
        $this->data['result'] = $this->os_model->getById($this->uri->segment(3));
        $this->data['tokenOs'] = $this->os_model->getProdutos($this->uri->segment(3));
        $this->data['produtos'] = $this->os_model->getProdutos($this->uri->segment(3));
        $this->data['servicos'] = $this->os_model->getServicos($this->uri->segment(3));
        $this->data['emitente'] = $this->mapos_model->getEmitente();
        $this->data['anexos'] = $this->os_model->getAnexos($this->uri->segment(3));
    ...
    public function dowloadanexotodos()
{
    $this->load->library('zip');
    $anexos = $this->db->get('anexos')->result();
    foreach ($anexos as $anexo) {
        $path = $anexo->path;
        $file_path = $path . '/' . $anexo->anexo;
        if (file_exists($file_path)) {
            $this->zip->read_file($file_path);
        }
    }
    $zip_file_name = 'anexos_' . date('d-m-Y') . '.zip';
    ...
    {
        // Dados específicos para a chamada
        $data = [
            'number' => '67996758663',
            'body'   => 'Teste de envio',
        ];

        // Chame a API usando a classe MyApi
        $response = $this->myapi->callApi($data);
    ...
    public function etiqueta()
    {
        ...
        $this->load->view('os/etiqueta', $this->data);
    }
